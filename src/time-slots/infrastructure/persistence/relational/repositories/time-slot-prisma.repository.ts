...
import {
  timeSlotInclude,
  TimeSlotMapperImplement,
} from '../mappers/time-slot-mapper.implement';

@Injectable()
export class TimeSlotRepositoryImplement implements TimeSlotRepository {
  ...
  async create(data: TimeSlot, user: User): Promise<TimeSlot> {
    ...
    return TimeSlotMapperImplement.toDomain(created);
  }
  ...
  async createMany(data: TimeSlot[], user: User): Promise<TimeSlot[]> {
    return this.prisma.$transaction(async (tx) => {
      ...
      return timeSlots.map((timeSlot) =>
        TimeSlotMapperImplement.toDomain(timeSlot),
      );
    });
  }
  ...
  async findAll(): Promise<TimeSlot[]> {
    ...
    return rows.map((row) => TimeSlotMapperImplement.toDomain(row));
  }
  ...
  async findById(id: TimeSlot['id']): Promise<NullableType<TimeSlot>> {
    ...
    return found ? TimeSlotMapperImplement.toDomain(found) : null;
  }
