...
import { TopicsMapperImplement } from '../mappers/topics-mapper.implement';

@Injectable()
export class TopicsRepositoryImplement implements TopicsRepository {
  ...
  async create(data: Topics): Promise<Topics> {
    ...
    return TopicsMapperImplement.toDomain(created);
  }
  ...
  async findAllWithPagination({
    ...
    return {
      data: rows.map((row) => TopicsMapperImplement.toDomain(row)),
      total,
    };
  }
  ...
  async findById(id: Topics['id']): Promise<NullableType<Topics>> {
    ...
    return found ? TopicsMapperImplement.toDomain(found) : null;
  }
  ...
  async update(id: Topics['id'], payload: Partial<Topics>): Promise<Topics> {
    ...
    return TopicsMapperImplement.toDomain(updated);
  }
  ...
  async findByIds(ids: Topics['id'][]): Promise<Topics[]> {
    ...
    return rows.map((row) => TopicsMapperImplement.toDomain(row));
  }
}
