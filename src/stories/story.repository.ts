...
function toDomainTopic(raw: TopicRow): Topic {
...
function toDomainUser(raw: HumanBookWithRelations): User {
  const user = new User();
  user.id = raw.id;
...
export function storyToDomain(raw: StoryWithRelations): Story {
  const story = new Story();
  story.id = raw.id;
  story.title = raw.title;
  story.abstract = raw.abstract;
  story.cover = toDomainFile(raw.cover);
  story.humanBook = toDomainUser(raw.humanBook);
  story.topics = raw.topics.map((storyTopic) =>
    toDomainTopic(storyTopic.topic),
  );
...
export class StoryRepository {
  ...
  async findAllWithCountAndPagination({
    ...
    if (hasFavoriteSort && currentUserId) {
      ...
      return {
        data: page.map((entity) => storyToDomain(entity)),
    ...
    return {
      data: entities.map((entity) => storyToDomain(entity)),
      count,
    };
  }
  ...
  async findMostPopularWithCountAndPagination({
    ...
    return {
      data: pageIds.map((id) => storyToDomain(entityById.get(id)!)),
  ...
  async findById(id: Story['id']): Promise<NullableType<Story>> {
    ...
    return entity ? storyToDomain(entity) : null;
  }
  ...
  async findRelatedTopics(id: Story['id']): Promise<Topic[]> {
    ...
    return (
      story?.topics.map((storyTopic) => toDomainTopic(storyTopic.topic)) ?? []
    );
  }
